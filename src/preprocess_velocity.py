...
# New namedtuple to hold opponent pose and velocity
OpponentData = namedtuple("OpponentData", ["pose", "velocity"])
...
def add_opponent_history(history, vehicle_id, timestamp, data, threshold):
    """
    Add a (timestamp, data) pair for a given vehicle into the opponent history.
    Here data is an OpponentData (contains pose and velocity).
    Immediately trim entries older than (timestamp - threshold).
    """
    if vehicle_id not in history:
        history[vehicle_id] = deque()
    history[vehicle_id].append((timestamp, data))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "subdirectories 'images', 'labels', and 'velocities' inside this folder."
        ),
    )
    parser.add_argument(
        "--mode",
...
def main():
    ...
    VEL_SAVE_PATH = os.path.join(DATASET, "velocities")  # New directory for velocities
    ...
    os.makedirs(VEL_SAVE_PATH)

    reader = RosBagReader(MCAP_DIR, TOPIC_NAMES)

    radar_window = deque()
    radar_counts = defaultdict(int)
    # Opponent history now stores tuples (timestamp, OpponentData) per vehicle_id.
    ...
    for topic, msg, timestamp in tqdm(reader.read_messages()):
        ...
        if mode == "gt" and vehicle_odom_pattern.match(topic):
            ...
            if match:
                ...
                # Extract pose and opponent velocity from the message.
                opp_data = OpponentData(
                    pose=msg.pose.pose, velocity=msg.twist.twist.linear.x
                )
        ...
        if mode == "tracks" and "/opponent/tracks" in topic:
            ...
            for track in msg.tracks:
                ...
                # Assuming track messages have similar fields (pose and twist)
                opp_data = OpponentData(
                    pose=track.pose.pose, velocity=track.twist.linear.x
                )
                add_opponent_history(
                    opponent_history,
                    vehicle_id,
        ...
        if "/vehicle/uva_odometry" in topic:
            ...
            # Ensure that radar data from at least one sensor is available.
            if not (
                radar_counts.get("front", 0) > 0 or radar_counts.get("rear", 0) > 0
            ):
                continue

            # *** Use ego velocity to transform opponent velocities ***
            ego_velocity = msg.twist.twist.linear.x

            # Synchronize opponent data to the ego timestamp.
            synced_opponent_data = {}
            for vehicle_id, history_deque in opponent_history.items():
                for ts, opp_data in reversed(history_deque):
                    if abs(ego_time - ts) <= OPPONENT_SYNC_THRESHOLD:
                        synced_opponent_data[vehicle_id] = opp_data
                        break
                    elif ts < ego_time - OPPONENT_SYNC_THRESHOLD:
                        break

            # If valid radar data and at least one synchronized opponent are available.
            if radar_window and synced_opponent_data:
                # Generate bounding boxes using opponent poses.
                bounding_boxes = preprocess_output(
                    msg.pose.pose,
                    {vid: data.pose for vid, data in synced_opponent_data.items()},
                )

                # Write labels (bounding boxes) as before.
                ...
                # Compute absolute opponent velocities.
                # (This simple addition assumes that the opponent velocities are reported relative to ego.)
                opponent_velocities = {}
                for vehicle_id, data in synced_opponent_data.items():
                    abs_velocity = data.velocity + ego_velocity
                    opponent_velocities[vehicle_id] = abs_velocity

                # Write the opponent velocities to a file.
                # Each line is "<vehicle_id> <absolute_velocity>"
                velocity_file = os.path.join(VEL_SAVE_PATH, f"{label_num}.txt")
                with open(velocity_file, "w") as vf:
                    for vehicle_id, abs_vel in opponent_velocities.items():
                        vf.write(f"{vehicle_id} {abs_vel:.6f}\n")
